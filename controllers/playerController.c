...
char* predmtNaipe(char* destination, Card *cardGroup, const int handSize){
    ...
    for(int k = 0; k < 5; k++) suits[k].count = 0;

    // Contando os elementos a partir do naipe
    for(int i = 0; i < handSize; i++){
        if(strcmp(cardGroup[i].naipe, "♥") == 0) suits[hearts].count++;
        else if(strcmp(cardGroup[i].naipe, "♦") == 0)suits[diamonds].count++;
        else if(strcmp(cardGroup[i].naipe, "♣") == 0) suits[clubs].count++;
        else if(strcmp(cardGroup[i].naipe, "♠") == 0) suits[spades].count++;
    }

    for(int j = 0; j < 4; j++)
        if(suits[countSuit].count < suits[j].count) suits[countSuit] = suits[j];
